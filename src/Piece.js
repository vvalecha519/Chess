...
export default function Piece({ i, source, defaultPosition, chess, mP, callBack, color, promotionCallBack, pP, pieceType }) {
  ...
  if ( newPoint[1] === '8' && pieType === 'p' && color === 'white' || newPoint[1] === '1' && pieType === 'p' && color === 'black') {
  ...
  useEffect(()=>{
    ...
    if ( pP != null && color == pP.color && oldPoint == pP.location) {
      ...
      if ( color == 'white') document.getElementById(`piece-${i}-${color}`).src= whitePieces[pP.type]
      else document.getElementById(`piece-${i}-${color}`).src= blackPieces[pP.type]
